...
const sectionTypeEnum = z.enum(["listening", "reading", "writing", "speaking", "general"], {
    errorMap: () => ({
        message: "Loại phần thi không hợp lệ. Phải là: listening, reading, writing, speaking, general",
    }),
});
...
const sectionsRoutes = async (fastify) => {
    ...
    fastify.post("/", { preHandler: [authenticate, requireRoles("admin", "teacher")] }, async (request, reply) => {
        return reply.status(403).send({
            error: "Không thể tạo section thủ công. Sections được tạo tự động khi tạo bài thi.",
        });
    });
    // PUT /sections/:id
    fastify.put("/:id", { preHandler: [authenticate, requireRoles("admin", "teacher")] }, async (request, reply) => {
        const { id } = request.params;
        const data = handleValidation(updateSectionSchema.safeParse(request.body), request, reply);
        if (!data)
            return;
        const authService = new AuthorizationService(fastify.prisma);
        try {
            await authService.requireSectionAuthoringAccess(id, request.user.id, request.user.roles);
        }
        catch (err) {
            if (err.statusCode) {
