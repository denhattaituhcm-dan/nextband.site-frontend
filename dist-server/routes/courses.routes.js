...
const coursesRoutes = async (fastify) => {
    ...
    fastify.get("/", { preHandler: optionalAuthenticate }, async (request, reply) => {
        ...
        if (!currentUser) {
            ...
            const hasAdminOrTeacherRole = currentUser.roles?.some((r) => ["admin", "teacher"].includes(r));
    ...
    fastify.post("/", { preHandler: [authenticate, requireRoles("admin", "teacher")] }, async (request, reply) => {
        const data = handleValidation(createCourseSchema.safeParse(request.body), request, reply);
        if (!data)
            return;
    ...
    fastify.put("/:id", { preHandler: [authenticate, requireRoles("admin", "teacher")] }, async (request, reply) => {
        const { id } = request.params;
        const data = handleValidation(updateCourseSchema.safeParse(request.body), request, reply);
        if (!data)
            return;
    ...
    fastify.delete("/:id", { preHandler: [authenticate, requireRoles("admin")] }, async (request, reply) => {
        ...
        if (enrollmentCount > 0 || submissionCount > 0) {
            return reply.status(409).send({
                error: "Không thể xóa khóa học khi vẫn còn học viên hoặc bài nộp liên quan",
            });
        }
