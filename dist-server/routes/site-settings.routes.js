...
function normalizeSettings(record) {
    const val = record && typeof record.value === "object" && record.value !== null
        ? record.value
        : record || {};
    return {
        id: record?.id || "global",
        ...
        authFeatureOneDescription: val.authFeatureOneDescription ||
            "Hàng trăm bài học từ cơ bản đến nâng cao",
        authFeatureTwoTitle: val.authFeatureTwoTitle || "Giáo viên uy tín",
        authFeatureTwoDescription: val.authFeatureTwoDescription || "Đội ngũ giáo viên giàu kinh nghiệm",
        ...
        heroDescriptionText: val.heroDescriptionText ||
...
const siteSettingsRoutes = async (fastify) => {
    fastify.get("/", async () => {
    ...
    fastify.put("/", { preHandler: [authenticate, requireRoles("admin")] }, async (request, reply) => {
        ...
        if (!parseResult.success) {
            ...
            const unrecognizedKeys = issues
                ...
                .flatMap((i) => i.keys || []);
            const errorMsg = unrecognizedKeys.length > 0
        ...
        const currentValue = current && typeof current.value === "object" && current.value !== null
            ? current.value
            : {};
        const newValue = {
            ...currentValue,
            ...Object.fromEntries(Object.entries(body).filter(([, val]) => val !== undefined)),
        };
