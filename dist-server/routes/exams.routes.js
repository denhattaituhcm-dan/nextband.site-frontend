...
const examsRoutes = async (fastify) => {
    ...
    fastify.post("/", { preHandler: [authenticate, requireRoles("admin", "teacher")] }, async (request, reply) => {
        const data = handleValidation(createExamSchema.safeParse(request.body), request, reply);
        if (!data)
            return;
    ...
    fastify.put("/:id", { preHandler: [authenticate, requireRoles("admin", "teacher")] }, async (request, reply) => {
        const { id } = request.params;
        const data = handleValidation(updateExamSchema.safeParse(request.body), request, reply);
        if (!data)
            return;
        const { isLocked: _ignoredIsLocked, ...safeData } = data;
        const authService = new AuthorizationService(fastify.prisma);
        try {
            await authService.requireExamAuthoringAccess(id, request.user.id, request.user.roles);
        }
        catch (err) {
            if (err.statusCode) {
    ...
    fastify.delete("/:id", { preHandler: [authenticate, requireRoles("admin")] }, async (request, reply) => {
        ...
        if (submissionCount > 0) {
            ...
            return reply.status(409).send({
                ...
                message: "Đề thi đã có bài làm của học viên. Hệ thống đã tự động chuyển sang chế độ Lưu trữ (Archived) để bảo toàn 100% lịch sử.",
                submissionCount,
            });
        }
