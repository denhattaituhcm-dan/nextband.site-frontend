...
import { SubmissionStateMachine, StateTransitionError } from "./submission-state-machine.js";
import { NotificationService } from "./notification.service.js";
import { getClassStudentIds, getTeacherStudentIds, isTeacherOfClass, } from "../utils/teacherScope.js";
const MANUAL_TYPES = new Set(["essay", "speaking"]);
const MAX_EXAM_ATTEMPTS = 3;
export function getRemainingSeconds(startedAt, durationMinutes) {
...
export class ExamSubmissionService {
            ...
            submission.exam = {
                ...
                sections: submission.exam.sections.map((sec) => {
                    ...
                    sanitizedSec.questionGroups = sec.questionGroups?.map((g) => ({
                        ...g,
                        questions: g.questions?.map((q) => sanitizeQuestionForStudent(q, canSeeSecrets)),
                    }));
                    return sanitizedSec;
                }),
            };
    ...
    async submitExam(user, id, payload) {
        ...
        const gradingSummary = canonicalScoringService.evaluateExamAttempt(examStructure, answersToEvaluate);
        ...
        return this.repo.transaction(async (tx) => {
            ...
            if (tx.notification) {
                ...
                if (targetStatus === "SUBMITTED") {
                    ...
                    const bandText = gradingSummary.bandScore !== null &&
    ...
    async gradeManualSubmission(user, id, grades, totalScore, options) {
        ...
        return this.repo.transaction(async (tx) => {
            ...
            const finalTotalScore = typeof totalScore === "number"
                ? totalScore
                : allAnswers.reduce((sum, a) => sum + (Number(a.score) || 0), 0);
            const updated = await tx.examSubmission.update({
                where: { id },
                data: {
                    status: "GRADED",
    ...
    async regradeSubmission(user, id, data) {
        ...
        return this.repo.transaction(async (tx) => {
            ...
            if (data.regradeAll) {
                ...
                const gradingSummary = canonicalScoringService.evaluateExamAttempt(submission.exam, rawAnswers);
