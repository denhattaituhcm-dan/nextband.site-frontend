...
export class AttendanceService {
    ...
    async getSessionAttendance(classId, sessionId, userId, userRoles = ['student']) {
        ...
        if (!session || session.classId !== classId)
            throw new NotFoundError('Buổi học không hợp lệ hoặc không thuộc lớp này.');
    ...
    async markSessionAttendance(classId, sessionId, teacherId, userRoles, items) {
        ...
        if (session.status === ClassSessionStatus.COMPLETED && !isAdmin) {
            throw new AuthorizationError('SESSION_ALREADY_COMPLETED: Buổi học đã chốt điểm danh (COMPLETED). Bạn không có quyền chỉnh sửa ngoại trừ Admin.', 403);
        }
        ...
        if (invalidStudentIds.length > 0) {
            throw new AuthorizationError(`INVALID_ENROLLMENT_STUDENT: Phát hiện học viên (${invalidStudentIds.join(', ')}) không thuộc danh sách học viên đang hoạt động của lớp học này.`, 400);
        }
    ...
    async completeSession(classId, sessionId, userId, userRoles) {
        ...
        if (unmarkedStudentIds.length > 0) {
            throw new AuthorizationError(`COMPLETION_GUARD_FAILED: Còn ${unmarkedStudentIds.length} học viên chưa được điểm danh (trạng thái UNMARKED). Bạn phải điểm danh 100% học viên trước khi hoàn thành buổi học.`, 400);
        }
    ...
    async getAttendanceMatrix(classId, userId, userRoles = ['student']) {
        ...
        const matrix = classStudents.map((cs) => {
            ...
            let excusedCount = 0;
            sessions.forEach((s) => {
                const att = studentAttendance.find((a) => new Date(a.sessionDate).toISOString().slice(0, 10) === new Date(s.plannedDate).toISOString().slice(0, 10));
        ...
        const actualMarkedRecords = allAttendance.filter((a) => {
            const isCompletedDate = completedSessions.some((cs) => new Date(cs.plannedDate).toISOString().slice(0, 10) === new Date(a.sessionDate).toISOString().slice(0, 10));
