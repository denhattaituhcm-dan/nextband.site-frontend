...
async function seed() {
  console.log("🌱 Seeding core IELTS courses...");
  let count = 0;
  for (const c of COURSES) {
    const { error } = await supabase.from("courses").upsert(c, { onConflict: "id" });
    if (error) {
      console.error(`❌ Failed to seed course ${c.title}:`, error.message);
    } else {
      count++;
    }
  }
  console.log(`✅ Seeding complete: ${count}/${COURSES.length} courses seeded.`);
}
