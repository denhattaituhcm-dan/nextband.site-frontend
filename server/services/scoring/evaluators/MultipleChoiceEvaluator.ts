...
export class MultipleChoiceEvaluator implements IQuestionEvaluator {
  ...
  private evaluateMultiSelect(
    ...
    if (typeof rawCorrect === "string" && rawCorrect.trim().startsWith("[")) {
      correctList = normalizer.parseJsonSafe<string[]>(rawCorrect, []);
    } else {
      correctList = normalizer.normalizeAlternatives(rawCorrect);
    }

    const expectedCount =
      typeof question.maxSelections === "number" && question.maxSelections > 0
