import { SubmissionGradingSummary, StudentRawAnswer } from "./types.js";

export class ResultBuilder {
  /**
   * Builds the database persistence payload for Prisma transaction
   */
  public buildDatabasePersistencePayload(
    submissionId: string,
    summary: SubmissionGradingSummary,
    rawAnswers: StudentRawAnswer[],
  ) {
    ...
    const answerUpdates = summary.evaluatedAnswers.map((evalRes) => {
      ...
      const answerTextVal =
        rawAns?.answerText !== undefined && rawAns?.answerText !== null
   ...
   */
  public buildClientResponse(submissionId: string, summary: SubmissionGradingSummary) {
    return {
      id: submissionId,
      status: summary.status,
