import { ScoringQuestion, StudentRawAnswer } from "./types.js";

export interface ResolvedExamAttempt {
  questions: ScoringQuestion[];
  answerMap: Map<string, StudentRawAnswer>;
  rawAnswerList: StudentRawAnswer[];
}
...
export class AnswerResolver {
  ...
  public resolve(
    ...
    const answerMap = new Map<string, StudentRawAnswer>();
    const rawAnswerList: StudentRawAnswer[] = [];

    if (Array.isArray(studentAnswers)) {
      for (const a of studentAnswers) {
        if (!a || !a.questionId) continue;
        const normalizedItem: StudentRawAnswer = {
