...
export interface CanonicalQuestionData {
  groupId: string;
...
export function sanitizeBackendQuestionPayload(input: any): {
  groupId: string;
  questionType: CanonicalQuestionType;
  questionText: string;
  options: string[] | null;
  ...
  const type = (input.questionType || "short_answer") as CanonicalQuestionType;
  ...
  let sanitizedCorrectAnswer: string | null =
    typeof input.correctAnswer === "string" ? input.correctAnswer.trim() : null;
